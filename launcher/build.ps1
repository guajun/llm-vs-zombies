...
& $compiler -std=c++20 -O2 -static -I (Join-Path $projectRoot "runtime/vendor") -municode (Join-Path $PSScriptRoot 'native_launcher.cpp') -o (Join-Path $OutputDirectory 'lvz-launcher.exe')
if ($LASTEXITCODE -ne 0) { throw 'Native launcher build failed.' }
& $compiler -std=c++20 -O2 -shared -static (Join-Path $PSScriptRoot 'bootstrap.cpp') (Join-Path $PSScriptRoot 'silent_audio.cpp') -lbcrypt -luser32 -ladvapi32 -lshell32 -o (Join-Path $OutputDirectory 'lvz-bootstrap.dll')
