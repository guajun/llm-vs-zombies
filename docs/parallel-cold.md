...
## 与并行分支 worker（#38 / N10）的边界

本文的并行对象始终是**同一条源轨迹的冷重放**，属计分管线：`cold_workers` 只控制同一种子的冷重放并发数，源录制、不同种子与恢复探针仍顺序执行，门槛（每 cold 的全部 `LIVE_GATES`、10 次冷启动、两旗周期）一条都不放宽。训练管线的"同根多分支并行"是另一条管线，见 [`并行分支worker.md`](并行分支worker.md)（issue #38）：每分支一个 worker 进程、一份用户档、一份日志与设备租约，且**首败不停派**。两条管线共用 PID/创建时间绑定、自有 Job 与只读回执的写法，但门槛与失败策略分开定义；分支 worker 既不复用也不替代本文的任何冷重放结论。
