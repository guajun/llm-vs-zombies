...
|---|---|---|
| `issue111-d-hosted-off-a` | `--mode on --probes off --single-cold`，`LVZ_LIFECYCLE_RECORDING=1`、`LVZ_LIFECYCLE_PROBES=0` | 探针关闭对照冷启动 A（仅差探针安装） |
| `issue111-d-hosted-off-b` | 同上 | 探针关闭对照冷启动 B（独立复跑） |
| `issue111-d-hosted-on-a` | `--mode on --probes on --single-cold`，`LVZ_LIFECYCLE_RECORDING=1`、`LVZ_LIFECYCLE_PROBES=1` | 探针开启冷启动 A |
| `issue111-d-hosted-on-b` | 同上 | 探针开启冷启动 B（独立复跑） |
