...
最新实机进展：035已自然触达同Board、UI3→4、GameClock不增的真实终局，完整封包10,786个边界及重连原ID恢复通过；#11仍等待其冷重放。033两组1,500 tick诊断的126,285次MT调用全部相同，但插桩轨迹较旧031已在post1,260分叉，不能据此关闭#12。具体证据和限制见[实机记录](headless-validation.md#随机调用诊断与真实零时钟终局)。
