...
- `initialize`: params `{game_mode:13,cards:[...]}`, with `expect`; valid only from the main menu without a Board. Supply exactly all available seed slots. The immediate result confirms configuration acceptance only; poll `observe.initialization.state` for `ready` or `error`. A request to enter a level is not proof that the level loaded or that its scene is correct.
